...
const generateOtherPaymentReportPdf = async ({ payments, academicYear, filters }) => {
  ...
  payments.forEach((payment, index) => {
    ...
    const isManual = payment.manualStudentDetails ? 'Manual' : 'Referenced';
    tableRows += `
      <tr>
        <td>${index + 1}</td>
  ...
  const studentTypeLabel = filters.studentType === 'manual' ? 'Manually Added Students' :
                          filters.studentType === 'referenced' ? 'Referenced Students' : 'All Students';

  const html = `
    <!DOCTYPE html>
    <html>
    <head>
      <title>Other Payments Report - ${studentTypeLabel}</title>
      <style>
    ...
    <body>
      <div class="header">
        ...
        <h2 class="document-title">OTHER PAYMENTS FINANCIAL REPORTS - ${studentTypeLabel}</h2>
      ...
      <div class="filters">
        ...
        ${filters.gradeLevel ? ` | Grade Level: ${filters.gradeLevel}` : ''}
        ${filters.classSection ? ` | Class Section: ${filters.classSection}` : ''}
        ${filters.studentType ? ` | Student Type: ${studentTypeLabel}` : ''}
        </p>
      </div>
      ...
      <table>
        ...
        <tbody>
          ${tableRows}
          <tr class="total-row">
            <td colspan="9" class="center">Total Amount:</td>
...
module.exports = generateOtherPaymentReportPdf;